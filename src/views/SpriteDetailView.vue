...
import { localizeSpriteName, localizeSpriteType, localizeSpriteVariant } from '../utils/spriteNames'
...
<template><main class="page-shell detail-page"><RouterLink class="back-link" to="/"><ArrowLeft :size="18" />Tous les Sprites</RouterLink>
  <template v-if="sprite"><section class="detail-hero"><SpriteImage :src="sprite.image" :alt="localizeSpriteName(sprite.name)" :lazy="false" /><p class="eyebrow">Esprit</p><h1>{{ localizeSpriteName(sprite.name) }}</h1><div class="detail-tags"><RarityBadge :rarity="sprite.rarity" /><Badge v-if="sprite.variant" variant="secondary" class="plain-badge">{{ localizeSpriteVariant(sprite.variant) }}</Badge></div><div class="status-row detail-status"><StatusBadge kind="owned" :active="isOwned(sprite.id)" @toggle="toggleOwned(sprite.id)" /><StatusBadge kind="mastered" :active="isMastered(sprite.id)" @toggle="toggleMastered(sprite.id)" /></div></section>
    <div class="detail-content"><section v-if="sprite.description" class="detail-section"><p class="eyebrow">Description</p><p class="description">{{ sprite.description }}</p></section><section class="detail-section"><p class="eyebrow">Informations</p><dl><InfoRow label="Type" :value="sprite.spriteType ? localizeSpriteType(sprite.spriteType) : undefined" /><InfoRow label="Variante" :value="sprite.variant ? localizeSpriteVariant(sprite.variant) : undefined" /><InfoRow label="Localisation" :value="sprite.location" /><InfoRow label="Coût d'invocation" :value="sprite.summonCost?.toLocaleString('fr-FR')" /><InfoRow label="Taux global" :value="sprite.percentage !== undefined ? `${sprite.percentage}%` : undefined" /></dl></section><section v-if="sprite.dropChances?.length" class="detail-section"><p class="eyebrow">Chances d'obtention</p><dl><InfoRow v-for="drop in sprite.dropChances" :key="drop.source" :label="drop.source" :value="typeof drop.chance === 'number' ? `${drop.chance}%` : drop.chance" /></dl></section><a class="source-link" :href="sprite.sourceUrl" target="_blank" rel="noopener noreferrer">Voir la source sur Fortnite.gg<ExternalLink :size="15" /></a></div>
