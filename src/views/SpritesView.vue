...
const { collection, isOwned, isMastered, toggleOwned, toggleMastered, replaceCollection, ownedCount, masteredCount } = useSpriteCollection()
const sync = useCollectionSync(collection, replaceCollection)
const filters = useSpriteFilters(sprites, isOwned, isMastered)
const filtersOpen = ref(false)
</script>
<template><a class="skip-link" href="#collection">Aller à la collection</a><AppHeader :displayed="filters.filteredSprites.value.length" :total="sprites.length" :owned="ownedCount" :mastered="masteredCount"><CollectionSync :status="sync.status.value" :configured="sync.isConfigured" :sync-code="sync.getSyncCode()" @sync="sync.synchronize" @import-code="sync.importSyncCode" /></AppHeader><main id="collection" class="page-shell collection-page">
